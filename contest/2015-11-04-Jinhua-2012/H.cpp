  	...
  	vector<Point2> convex;
    {
        ...
        for (int i = 0; i < (int)point2.size(); ++i) {
            while ((int)stack.size() >= 2 &&
                    sign(det(stack[(int)stack.size() - 2], stack.back(), point2[i])) <= 0) {
                stack.pop_back();
            }
...
pair<double, double> getanswer(int vertex, int to) {
	double dist = fabs(volume(vertex, face[to][0], face[to][1], face[to][2])) / fabs(area(face[to][0], face[to][1], face[to][2])); 
