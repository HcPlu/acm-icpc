...
void solve() {
	...
	if (2 * radiusa - width > 2 * 1e-4 || 2 * radiusa - height > 2 * 1e-4 || 2 * radiusb - width > 2 * 1e-4 || 2 * radiusb - height > 2 * 1e-4) {
		puts("impossible");
		return;
	}
	...
	while (times--) {
		...
		long double now = sa * anglea / 2 / pi - radiusa * radiusa * sina / 2 + sb * angleb / 2 / pi - radiusb * radiusb * sinb / 2;
...
int main() {
	for (; scanf("%lf%lf%lf%lf%lf", &width, &height, &sa, &sb, &area) && (width + height + sa + sb + area > eps); solve());	
}
