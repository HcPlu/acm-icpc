...
void solve() {
	...
	for (;point[left_state.first].first + left_state.second < L + eps && left_state.first < n && right_state.first < n; ) {
		fix(left_state); fix(right_state);
		...
		if (left != right) {
			...
			long double b1 = point[left].second + k1 * left_state.second, b2 = point[right].second + k2 * right_state.second;
			if (sign(k1 - k2) != 0) {
