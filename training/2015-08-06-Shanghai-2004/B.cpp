...
bool move() {
    ...
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= m; j++) {
            ...
            if (moved[i][j] || mat[tx][ty] != '*' || tx < 1 || tx > n || ty < 1 || ty > m)
                continue;
            mat[tx][ty] = mat[i][j];
...
bool check2(int x1, int y1, int x2, int y2) {
    ...
    int tx = x1, ty = y2;
    if (mat[tx][ty] == '*') {
        flg |= (sumx[x1][max(ty, y1) - 1] == sumx[x1][min(ty, y1)]) &&
               (sumy[y2][max(x1, x2) - 1] == sumy[y2][min(x1, x2)]);
    }
    tx = x2, ty = y1;
    if (mat[tx][ty] == '*') {
        flg |= (sumx[x2][max(ty, y2) - 1] == sumx[x2][min(ty, y2)]) &&
               (sumy[y1][max(x1, x2) - 1] == sumy[y1][min(x1, x2)]);
    }
    return flg;
}
...
bool check3(int x1, int y1, int x2, int y2) {
    ...
    for (int i = x1 + 1; i <= n + 1; i++) {
        if (mat[i][y1] != '*' || mat[i][y2] != '*')
            continue;
        if (sumy[y1][x1] != sumy[y1][i])
            break;
        if (i == x2)
            continue;
        if (i < x2)
            if (sumy[y2][x2] - sumy[y2][i] > 1)
                continue;
        if (i > x2)
            if (sumy[y2][i] != sumy[y2][x2])
                break;
        if (sumx[i][min(y1, y2) - 1] != sumx[i][max(y1, y2)])
            continue;
        return 1;
    }
    ...
    for (int i = y1 + 1; i <= m + 1; i++) {
        if (mat[x1][i] != '*' || mat[x2][i] != '*')
            continue;
        if (sumx[x1][i] != sumx[x1][y1])
            break;
        if (i == y2)
            continue;
        if (i < y2)
            if (sumx[x2][y2] - sumx[x2][i] > 1)
                continue;
        if (i > y2)
            if (sumx[x2][i] != sumx[x2][y2])
                break;
        if (sumy[i][min(x1, x2)] != sumy[i][max(x1, x2) - 1])
            continue;
        return 1;
    }
    for (int i = y1 - 1; i >= 0; i--) {
        if (mat[x1][i] != '*' || mat[x2][i] != '*')
            continue;
        if (sumx[x1][y1] - sumx[x1][i] > 1)
            break;
        if (sumx[x2][y2] - sumx[x2][i] > 1)
            break;
        if (sumy[i][min(x1, x2) - 1] != sumy[i][max(x1, x2)])
            continue;
        return 1;
    }
    return 0;
...
void chkmin(pair<int, pair<int, int> > &p1, pair<int, pair<int, int> > p2) {
    p1 = min(p1, p2);
}

bool lianliankan() {
    pair<int, pair<int, int> > nowans = make_pair(123, make_pair(INF, INF));
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= m; j++) {
            if (mat[i][j] != '*') {
                for (int p = i; p <= n; p++)
