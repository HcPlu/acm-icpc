...
void solve(void) {
    ...
    for (int len = 1; len <= k; ++len) {
        ...
        for (multimap<string, int>::const_iterator iter1 = list1.begin(); iter1 != list1.end(); ++iter1) {
            pair<multimap<string, int>::const_iterator, multimap<string, int>::const_iterator> range = list2.equal_range(
                    iter1->first);
            for (multimap<string, int>::const_iterator iter2 = range.first; iter2 != range.second; ++iter2) {
                if (iter1->second + (int)crib1.length() - 1 < iter2->second ||
                    iter2->second + (int)crib2.length() - 1 < iter1->second) {
                    if (answer == "") {
                        answer = iter1->first;
