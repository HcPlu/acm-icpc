...
pair<int, int> get(int x, int num) {
    pair<int, int> t = make_pair(0x3f3f3f3f, 0x3f3f3f3f);
    if (x == 0) {
        return t;
    }
    if (num == -1) {
    ...
    if (deep[now] <= deep[x] + w[x].first) {
        t = make_pair(deep[x] - w[x].first, w[x].second);
    }
