...
bool check(int index) {
    ...
    for (int dir = 0; dir < 4; ++dir) {
        ...
        for (int i = 0; i < m; ++i) {
            ...
            if (dir == 0) {
                if (lower[i].x1 >= lower[index].x2 &&
                    intersect(lower[i].y1, lower[i].y2, lower[index].y1, lower[index].y2)) {
                    clear = false;
                    break;
                }
            }
            if (dir == 1) {
                if (lower[i].x2 <= lower[index].x1 &&
                    intersect(lower[i].y1, lower[i].y2, lower[index].y1, lower[index].y2)) {
                    clear = false;
                    break;
                }
            ...
            if (dir == 2) {
                if (lower[i].y1 >= lower[index].y2 &&
                    intersect(lower[i].x1, lower[i].x2, lower[index].x1, lower[index].x2)) {
                    clear = false;
                    break;
                }
            }
            if (dir == 3) {
                if (lower[i].y2 <= lower[index].y1 &&
                    intersect(lower[i].x1, lower[i].x2, lower[index].x1, lower[index].x2)) {
                    clear = false;
                    break;
                }
