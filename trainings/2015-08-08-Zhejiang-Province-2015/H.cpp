    ...
    for (int counter = 1;; counter++) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (dist(a[pos.first].center, a[i].center) < total + eps) {
                double alpha = compute(dist(a[pos.first].center, a[i].center), a[pos.first].radius + coin.radius,
        ...
        pos = std::make_pair(best.second, fix(atan2(a[best.second].center, a[pos.first].center) -
                                              compute(dist(a[pos.first].center, a[best.second].center),
